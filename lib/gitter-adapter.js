...
function nickToMask(nick) {
  return ':' + nick + '!' + nick + '@irc.gitter.im';
}
...
Adapter.prototype.getOurRoom = function getOurRoom(room) {
...
Adapter.prototype.subscribeToRoom = function(room) {
  ...
  var ourroom = this.getOurRoom(room);
  ourroom._users = {};
  ourroom.hasUser = function hasUser(nick) {
    ourroom._users[nick] = true;
  }
  ourroom.addUser = function addUser(nick) {
    if (nick in ourroom._users)
      return;
    this.hasUser(nick);
    c.send(nickToMask(nick), 'JOIN', '#' + room.uri);
  }
  ourroom.delUser = function delUser(nick) {
    if (nick in ourroom._users) {
      delete ourroom._users[nick];
      c.send(nickToMask(nick), 'PART', '#' + room.uri);
    }
  }
